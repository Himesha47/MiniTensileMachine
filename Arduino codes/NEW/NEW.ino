...
void handleSerialCommands() {
  ...
  if (cmd == "START") {
    ...
    if (!runningTest) {
      digitalWrite(PIN_DIR, LOW);  // Set hardware direction pin LOW = DOWN
      dirSign = -1; // Update direction sign for step counting
      Serial.println("OK DIR_DOWN");
    } else {
      // Tell Python the command was rejected so user knows to press STOP first
      Serial.println("ERR STOP_FIRST");
    }
  }
}
...
void loop() {
  ...
  // Safety limit — stop motor and notify Python
  if (travelLimitReached(disp_mm)) {
    runningTest = false; // Stop the test loop
    disableDriver();  // Cut power to the stepper driver
