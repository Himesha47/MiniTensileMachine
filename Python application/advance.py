...
class TensileGUI:
    ...
    def set_dir(self, direction):
        # Change the motor direction — only allowed when the test is not running
        ...
        if self.ser and self.ser.is_open:   # Only proceed if connected
            try:
                ...
                while self.ser.in_waiting:  # in_waiting = number of bytes waiting to be read
                    ...
                    # Only store and process data once we have 15 readings to average
                    if len(self.force_window) == self.force_window.maxlen:
                        avg_f = sum(self.force_window) / 15  # Averaged force in N
                        avg_d = sum(self.disp_window)  / 15  # Averaged displacement in mm
                        ...
                        if (self.break_force is None
                                and self.peak_force > 0.5        # only arm after 0.5 N peak 
                        ...
                        # ------- Update the six metric cards -------
                        self.card_force.set(f"{avg_f:7.2f}") # 2 decimal places
                        self.card_disp.set(f"{avg_d:7.3f}") # 3 decimal places
                        ...
                        if self.youngs_mod is not None:
                            self.card_E.set(f"{self.youngs_mod:,.0f}") # No decimal, comma separator
